...
export function createPageImageHandlers({ listImages, saveImage }) {
...
function failure(error) {
  ...
  console.error("Azura sayfa görsel API hatası:", error);
  return json({ error: "Azura sayfa görselleri işlenemedi." }, 500);
}
...
async function GET(request) {
  ...
  try { return json({ images: await listImages() }); }
...
async function POST(request) {
  ...
  try {
    ...
    return json(await saveImage(Buffer.from(await file.arrayBuffer()), file.type), 201);
  } catch (error) { return failure(error); }
}
