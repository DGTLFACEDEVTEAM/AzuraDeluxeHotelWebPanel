...
test('Family yönetim izin listesi, kesin oda şeması ve medya sahipliği korunur',async t=>{
 const {paths,file}=await fixture(t);assert.equal(roomDetailApiConfig('family').pageKey,'familyroom');assert.throws(()=>roomDetailApiConfig('fantasy'),e=>e.status===404);assert.match((await readRoomDetailPageContent('family',paths)).revision,/^[a-f0-9]{64}$/);await assert.rejects(writeRoomDetailPageContent('family',{}, {},'x',paths),e=>e.status===400);
