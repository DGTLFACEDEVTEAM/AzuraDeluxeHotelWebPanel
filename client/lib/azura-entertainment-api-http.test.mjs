...
const urls = Object.fromEntries(["tr", "en", "de", "ru"].map(l => [l, `/${l}/entertainment`]));
const locales = ["tr", "en", "de", "ru"];
const token = "azura-entertainment-test-service-token-123456789";
...
async function startServer(port, paths) {
  ...
  for (let i = 0; i < 100; i++) {
    ...
    try {
      const response = await fetch(`${base}/uploads/pages/entertainment/ent_ban.jpg`);
...
test("production Entertainment API'leri, dört dil, paralel kayıt, medya ve kalıcılık", { timeout: 120000 }, async (t) => {
  const root = await mkdtemp(path.join(os.tmpdir(), "azura-entertainment-http-"));
  t.after(() => rm(root, { recursive: true, force: true }));
  const paths = { contentRoot: path.join(root, "content"), uploadsRoot: path.join(root, "uploads") };
  const entertainmentFilePath = path.join(paths.contentRoot, "site-pages/entertainment.json");
  const uploads = path.join(paths.uploadsRoot, "pages/entertainment");
  await mkdir(path.dirname(entertainmentFilePath), { recursive: true });
  await mkdir(uploads, { recursive: true });
  await writeFile(entertainmentFilePath, JSON.stringify({ ...seed, futureMetadata: { kept: true } }));
  for (const name of await readdir(path.join(appRoot, "public/uploads/pages/entertainment"))) {
    await copyFile(path.join(appRoot, "public/uploads/pages/entertainment", name), path.join(uploads, name));
  }
  ...
  for (const record of entertainmentImages(seed.media)) {
  ...
  for (const locale of locales) {
    ...
    assert.ok(html.includes(escapeHtml(values.activities.title)), locale);
    assert.ok(html.includes(escapeHtml(values.gridSection.title1)), locale);
    assert.ok(html.includes(escapeHtml(values.gridSection.title)), locale);
    assert.ok(html.includes(seed.media.hero.image), locale);
    assert.ok(html.includes(seed.media.activities[0].image), locale);
  }

  const pageUrl = `${base}/api/azura/entertainment/page-content`;
  const imagesUrl = `${base}/api/azura/entertainment/images`;
  ...
  const originalBytes = await readFile(entertainmentFilePath);
  ...
  tooLong.tr.activities.title = "x".repeat(4001);
  ...
  delete missingList.tr.activities.text;
  ...
  assert.deepEqual(await readFile(entertainmentFilePath), originalBytes);
  for (const mutate of [
    m => { m.hero.translations = seed.media.activities[0].translations; },
    m => { delete m.gridSection[0].translations; },
    m => { m.activities[1].order = 0; },
    m => { m.activities[0].id = "nighttime"; },
    m => { m.gridSection.pop(); }, m => { m.gridSection.push(m.gridSection[0]); },
  ...
  for (const mutate of [
    b => { b.fr = b.tr; }, b => { b.en.activities.title = ""; },
    b => { b.tr.gridSection.text1 = ""; }, b => { b.de.activities.text = " "; },
  ]) { const b = structuredClone(snapshot.bundle); mutate(b); assert.equal((await put(b)).status, 400); }
  assert.deepEqual(await readFile(entertainmentFilePath), originalBytes);
  const beforeConcurrent = await readFile(entertainmentFilePath);
  assert.equal((await fetch(pageUrl, { headers: auth }).then((response) => response.json())).revision,
    snapshot.revision);
  assert.deepEqual(await readFile(entertainmentFilePath), beforeConcurrent);

  const first = structuredClone(snapshot.bundle);
  const second = structuredClone(snapshot.bundle);
  first.tr.activities.title = "Concurrent first";
  second.tr.activities.title = "Concurrent second";
  ...
  assert.deepEqual(JSON.parse(await readFile(entertainmentFilePath)).futureMetadata, { kept: true });
  assert.equal(JSON.parse(await readFile(entertainmentFilePath)).pageKey, "entertainment");

  await symlink(path.join(uploads, "ent_ban.jpg"), path.join(uploads, "linked.jpg"));
  const linked = structuredClone(afterConcurrent.media);
  linked.hero.image = "/uploads/pages/entertainment/linked.jpg";
  const beforeLinked = await readFile(entertainmentFilePath);
  assert.equal((await put(afterConcurrent.bundle, linked, afterConcurrent.revision)).status, 400);
  assert.deepEqual(await readFile(entertainmentFilePath), beforeLinked);
  ...
  assert.ok(listed.every((image) => image.image.startsWith("/uploads/pages/entertainment/") &&
  ...
  const bytes = await readFile(path.join(uploads, "ent_ban.jpg"));
  const beforeUpload = await readFile(entertainmentFilePath);
  const upload = await fetch(imagesUrl, { method: "POST", headers: auth, body: multipart(bytes) });
  assert.equal(upload.status, 201);
  assert.deepEqual(await readFile(entertainmentFilePath), beforeUpload);
  const uploaded = await upload.json();
  const afterUploadList = await fetch(imagesUrl, { headers: auth }).then(r => r.json());
  assert.equal(afterUploadList.images.length, 13);
  assert.ok(afterUploadList.images.some(r => r.image === uploaded.image));
  assert.match(uploaded.image, /^\/uploads\/pages\/entertainment\/entertainment-[a-f0-9-]+\.jpg$/);
  ...
  assert.equal(uploaded.width, 2880);
  ...
  for (const locale of locales) updated[locale].activities.title = `Azura entertainment update ${locale}`;
  const updatedMedia = structuredClone(afterConcurrent.media);
  updatedMedia.gridSection[0].image = uploaded.image;
  updatedMedia.gridSection[0].width = uploaded.width;
  updatedMedia.gridSection[0].height = uploaded.height;
  ...
  assert.equal(saved.media.gridSection[0].image, uploaded.image);
  assert.equal(saved.media.hero.image, uploaded.image);
  assert.equal(Object.hasOwn(saved.media.hero, "translations"), false);
  assert.deepEqual(saved.media.gridSection[0].translations, seed.media.gridSection[0].translations);
  assert.match(saved.revision, /^[a-f0-9]{64}$/);
  const beforeConflict = await readFile(entertainmentFilePath);
  assert.equal((await put(updated, updatedMedia, afterConcurrent.revision)).status, 409);
  assert.deepEqual(await readFile(entertainmentFilePath), beforeConflict);
  for (const locale of locales) {
    const response = await fetch(`${base}${urls[locale]}`);
    assert.equal(response.status, 200, locale);
    const html = await response.text();
    assert.ok(html.includes(`Azura entertainment update ${locale}`), locale);
  ...
  for (const locale of locales) {
    const response = await fetch(`${base}${urls[locale]}`);
    assert.equal(response.status, 200, locale);
    assert.ok((await response.text()).includes(`Azura entertainment update ${locale}`), locale);
  }
  const restored = await fetch(pageUrl, { headers: auth }).then(r => r.json());
  assert.deepEqual(restored, saved);
  const finalFile = JSON.parse(await readFile(entertainmentFilePath));
  assert.equal(finalFile.schemaVersion, 1); assert.equal(finalFile.pageKey, "entertainment"); assert.equal(Object.hasOwn(finalFile, "revision"), false);
  assert.deepEqual(JSON.parse(await readFile(entertainmentFilePath)).futureMetadata, { kept: true });
});
