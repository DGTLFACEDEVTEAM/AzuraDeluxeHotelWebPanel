...
function failure(error) {
  if (error instanceof ContentError || error instanceof HomepageContentError) {
    return json({ error: error.message }, error.status);
  }
  console.error("Azura sayfa sayfası API hatası:", error);
  return json({ error: "Azura sayfa sayfası işlenemedi." }, 500);
}

async function readLimitedBody(request) {
  if (Number(request.headers.get("content-length")) > MAX_BODY_BYTES) {
    throw new ContentError("İstek gövdesi çok büyük.", 413);
  }
...
async function GET(request) {
  ...
  try { return json(await readContent()); } catch (error) { return failure(error); }
}
