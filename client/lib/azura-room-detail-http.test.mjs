...
test("Deluxe dört dil: metinler, medya/öneri sırası, turlar, kalıcı yayın ve Family/Fantasy regresyonu", { timeout: 120000 }, async (t) => {
  ...
  for (const folder of ['deluxeroom','familyroom','room-options']) await cp(path.join(appRoot, 'public/uploads/pages', folder), path.join(paths.uploadsRoot, 'pages', folder), { recursive:true });
  ...
  for(const key of ['fantasy','handicap','unknown','constructor']) for(const suffix of ['page-content','images']) for(const method of ['GET',suffix==='images'?'POST':'PUT']) assert.equal((await fetch(`${base}/api/azura/room-details/${key}/${suffix}`,{method,headers:auth})).status,404);
  ...
  const listing=await (await requestAuth(mediaEndpoint)).json();assert.equal(listing.images.length,11);assert.ok(listing.images.every(i=>!i.image.includes("/familyroom/")));assert.ok(listing.images.some(i=>i.image==='/uploads/pages/room-options/fantasy-preview.jpg'));assert.ok(listing.images.every(i=>i.width&&i.height&&i.modifiedAt));
  ...
  for (const image of new Set([...roomDetailImages(seed.media),...roomDetailImages(familySeed.media)].map(r=>r.image))) {
  ...
  for (const locale of locales) {
    ...
    for (const [room,namespace,count] of [['fantasyroom','FantasyRoom',1],['handicaproom','HandicapRoom',1]]) {
  ...
  const familyList=await (await requestAuth(familyImages)).json();assert.equal(familyList.images.length,14);assert.ok(familyList.images.every(i=>!i.image.includes('/deluxeroom/')&&i.width&&i.height&&i.modifiedAt));assert.equal(familyList.images.filter(i=>i.image.includes('/room-options/')).length,2);
  ...
  await published();await stopServer(child);({child}=await startServer(port,paths));await published();await familyPublished();
