...
const temporary=await mkdtemp(path.join(os.tmpdir(),'azura-entertainment-production-'));
...
try {
 ...
 await run(['run','build']);
 await run(['run','test:entertainment-http']);
 await run(['run','test:bars-http']);
 await run(['run','test:kidsclub-http']);
