...
const Page = async ({ params }) => {
  ...
  const { texts, images, tours, rooms, featureTexts } = await readRoomDetailLocale('fantasy', locale)
  ...
  const iconTexts = [info.amenities.couples, info.amenities.kingBed, info.amenities.jacuzziTerrace]
  ...
  return (
    <div className=' overflow-hidden flex flex-col items-center justify-center gap-[60px] md:gap-[80px] lg:gap-[100px] bg-[#fbfbfb]'>
     ...
     <SubroomCarousel images={images.gallery}/>
      ...
      <RoomFeatures span={info.subtitle} header={info.title} text={info.text} header2={info.title2} header3={info.title3}  text2={info.text2} iconsTexts={iconTexts} features={featureTexts}  sofa={false} />
     <BackgroundSection span={background.subtitle} header={background.title} texts={[background.text, background.list1, background.list2]} link="/" img={images.background}/>
