...
function failure(error) {
  ...
  console.error("Azura homepage image API hatası:", error);
  return json({ error: "Azura homepage görselleri işlenemedi." }, 500);
}
...
export async function GET(request) {
  ...
  try {
    return json({ images: await listHomepageImages() });
  } catch (error) {
...
export async function POST(request) {
  ...
  try {
    ...
    const image = await saveHomepageImage(Buffer.from(await file.arrayBuffer()), file.type);
