...
import { RoomsContentError } from "@/lib/azura-rooms-storage.mjs";
import { readRoomsPageContent, writeRoomsPageContent } from "@/lib/azura-rooms-page-content.mjs";
...
function failure(error) {
  ...
  console.error("Azura oda sayfası API hatası:", error);
  return json({ error: "Azura oda sayfası işlenemedi." }, 500);
}
...
export async function GET(request) {
  ...
  try { return json(await readRoomsPageContent()); } catch (error) { return failure(error); }
}
...
export async function PUT(request) {
  ...
  try {
    ...
    const result = await writeRoomsPageContent(body.bundle, body.media, expectedRevision);
    for (const locale of LOCALES) {
      revalidatePath(`/${locale}/rooms`);
      revalidatePath(`/${locale}${routing.pathnames["/rooms"][locale]}`);
    }
    return json(result);
