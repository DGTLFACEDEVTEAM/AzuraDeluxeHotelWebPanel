...
function failure(error) {
  ...
  console.error("Azura rooms image API hatası:", error);
  return json({ error: "Azura oda görselleri işlenemedi." }, 500);
}
...
export async function GET(request) {
  ...
  try {
    return json({ images: await listRoomsImages() });
  } catch (error) {
...
export async function POST(request) {
  ...
  try {
    ...
    const image = await saveRoomsImage(Buffer.from(await file.arrayBuffer()), file.type);
