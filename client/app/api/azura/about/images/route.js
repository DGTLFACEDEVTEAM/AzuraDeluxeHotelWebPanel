...
import {
  ...
  listAboutImages, saveAboutImage,
...
function failure(error) {
  ...
  console.error("Azura Hakkımızda görsel API hatası:", error);
  return json({ error: "Azura Hakkımızda görselleri işlenemedi." }, 500);
}
...
export async function GET(request) {
  ...
  try { return json({ images: await listAboutImages() }); }
...
export async function POST(request) {
  ...
  try {
    ...
    return json(await saveAboutImage(Buffer.from(await file.arrayBuffer()), file.type), 201);
  } catch (error) { return failure(error); }
}
