...
import {
  ...
  listSpaWellnessImages, saveSpaWellnessImage,
...
function failure(error) {
  ...
  console.error("Azura Spa & Wellness görsel API hatası:", error);
  return json({ error: "Azura Spa & Wellness görselleri işlenemedi." }, 500);
}
...
export async function GET(request) {
  ...
  try { return json({ images: await listSpaWellnessImages() }); }
...
export async function POST(request) {
  ...
  try {
    ...
    return json(await saveSpaWellnessImage(Buffer.from(await file.arrayBuffer()), file.type), 201);
  } catch (error) { return failure(error); }
}
