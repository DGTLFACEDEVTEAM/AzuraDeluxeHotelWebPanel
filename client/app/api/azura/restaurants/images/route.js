...
import {
  ...
  listRestaurantsImages, saveRestaurantsImage,
...
function failure(error) {
  ...
  console.error("Azura restoran görsel API hatası:", error);
  return json({ error: "Azura restoran görselleri işlenemedi." }, 500);
}
...
export async function GET(request) {
  ...
  try { return json({ images: await listRestaurantsImages() }); }
...
export async function POST(request) {
  ...
  try {
    ...
    return json(await saveRestaurantsImage(Buffer.from(await file.arrayBuffer()), file.type), 201);
  } catch (error) { return failure(error); }
}
